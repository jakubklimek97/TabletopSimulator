...
public class InstancedMesh extends Mesh {
  ...
  //this(indices, positions, textureCoords, normals, Mesh.createEmptyIntArray(Mesh.MAX_WEIGHTS * positions.length / 3, 0), Mesh.c
    public InstancedMesh(int[] indices, float[] positions, float[] textureCoords,float[] normals, int numInstances) {
        super(indices, positions, textureCoords, normals, Mesh.createEmptyIntArray(Mesh.MAX_WEIGHTS * positions.length / 3, 0), Mesh.createEmptyFloatArray(Mesh.MAX_WEIGHTS * positions.length / 3, 0));
    ...
    public void renderListInstanced(List<Entity> gameItems, TransformManager transformation, Matrix4f viewMatrix) {
        renderListInstanced(gameItems, false, transformation, viewMatrix);
    }

    public void renderListInstanced(List<Entity> gameItems, boolean billBoard, TransformManager transformation, Matrix4f viewMatrix) {
        initRender();
    ...
    private void renderChunkInstanced(List<Entity> gameItems, boolean billBoard, TransformManager transformation, Matrix4f viewMatrix) {
        this.instanceDataBuffer.clear();

        int i = 0;

        TextureManager text = getMaterial().getTexture();
        for (Entity entity : gameItems) {
            ...
            if (text != null) {
                int col = entity.getTexturePos() % text.getNumCols();
                int row = entity.getTexturePos() / text.getNumCols();
            ...
            this.instanceDataBuffer.put(buffPos, billBoard ? entity.getScale() : entity.isSelected() ? 1 : 0);
